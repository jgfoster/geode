# Geode — GemStone Package Manager
...
Geode takes the position that **files on disk are the source of truth** and
...
```bash
git clone https://github.com/jgfoster/geode.git
cd geode
pip install -e .
```
...
Every Geode project has a `gemstone.toml` at its root. This is the file you
edit by hand.
...
If a dependency has neither a manifest nor load overrides, Geode auto-discovers
...
Geode uses Cargo-style version constraints:
...
Running `geode fetch` produces `gemstone.lock`, which records the exact
...
support. Geode scans for forward references between peer classes and
...
### `geode migrate-mcz <mcz-path>`

Best-effort one-way migration of a Monticello `.mcz` archive into a Geode
...
Geode can consume packages written in Tonel format (`.st` files — one file per
class), as used by Pharo and some GemStone projects. Since GemStone's Topaz
does not load `.st` files directly, Geode transpiles them to `.tpz` file-in
...
Geode auto-discovers all `.st` files in each listed directory and determines
...
When `geode install` or `geode test` runs:

1. For each dependency (and the project itself) that declares `load.tonel`
   directories, Geode parses every `.st` file in those directories.
...
3. The generated `.tpz` file is written to `.geode/tonel/<package>/` and
...
The `.geode/tonel/` directory is ephemeral and gitignored — it is regenerated
on each install.

This works for both the project's own source and fetched dependencies. If a
dependency's `gemstone.toml` declares `tonel` directories, Geode transpiles
them automatically.
...
Geode supports a lightweight package registry: a GitHub repository containing
...
`github.com/GemTalk/geode-index`).

You can also bypass the registry entirely by specifying `--git` URLs directly
in `geode add` or in your `gemstone.toml`.

## Configuration

Global configuration lives in `~/.geode/config.toml`:

```toml
registry_url = "https://github.com/jgfoster/geode-index"
...
Stone credentials configured here are used as defaults by `geode install`
and `geode test`, and can be overridden with `--user` and `--password`.

## How Geode Differs from Rowan
...
Geode takes a fundamentally different approach:
...
Geode deliberately omits image-to-disk write-back. You always edit files and
...
tight IDE integration and image-based development. Geode is the right choice
...
3. Upload to [TestPyPI](https://test.pypi.org) first to verify everything looks correct:
   ...
   pip install --index-url https://test.pypi.org/simple/ gemstone-geode
   ```
